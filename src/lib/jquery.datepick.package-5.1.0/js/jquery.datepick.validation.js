...
(function($) {
  ...
  function validateEach(value, element) {
    ...
    var dates = multiSelect
      ...
      : rangeSelect
      ? value.split(inst.get('rangeSeparator'))
      : [value];
    var ok =
      ...
      try {
        ...
        $.each(dates, function(i, v) {
          ...
          ok =
            ok &&
            (!dates[i] ||
              (dp.datepick('isSelectable', dates[i]) &&
    ...
    $.validator.addMethod(
      ...
      function() {
        ...
        return minDate && maxDate
          ? $.datepick.errorFormat(messages.validateDateMinMax, [
              minDate,
              maxDate,
            ])
          : minDate
          ? $.datepick.errorFormat(messages.validateDateMin, [minDate])
          : maxDate
          ? $.datepick.errorFormat(messages.validateDateMax, [maxDate])
      ...
      function(params) {
        ...
        return messages.validateDateCompare
          .replace(
            ...
            messages[
              'validateDate' +
                (comparisons[params[0]] || params[0]).toUpperCase()
            ]
          )
          .replace(/\{1\}/, thatDate);
      }
